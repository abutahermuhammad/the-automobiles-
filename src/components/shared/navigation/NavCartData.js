        ...
        <>
            ...
            <Stack gap={3} direction="vertical" style={{width: 300}}>
                <Stack className="mx-2" gap={3} direction="horizontal">
                    <Link to={`/cart`} className="w-50 btn btn-outline-dark btn-sm">
                        cart
                    </Link>

                    <Link to={`/checkout`} className="w-50 btn btn-primary btn-sm">
                        Checkout
                    </Link>
                </Stack>

                {(cart?.length > 0) && (
                    <Button className="mx-2" variant="primary" size="sm" onClick={() => clearCart()}>
                        Clear Cart
                    </Button>
                )}
